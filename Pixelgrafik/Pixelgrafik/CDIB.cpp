...
CDIB* CDIB::fft() {
	...
	if (fourier) {
		...
		temp = new CDIB(b, h);  // Bild mit vorangegangerner Breite, Höhe
		m_fft.FFT2D(-1, 0);
...
//+ fftrealvektor
//Vektor mit real-anteilen für fft aus bild herstellen
//COLOR für farbkanal, 0-blau,1-grün,2-rot
...
//+ fftimagvektor
//imaginär-vektor für fft zurückgeben, mit nullen gefüllt
double* CDIB::fftimagvektor()
{
	...
	//leeren Vektor erzeugen, keine imaginär Anteile
	memset(vektor, 0, (height_pow2 * width_pow2 + 1) * sizeof(double));
...
//+ fftcopy
// kopiere die real und imaginär ergebnisse der fft in ein dib zur visualisierung
...
void CDIB::fftcopy(double* real, double* image, int COLOR)
{
	...
	//vorwärts-fft, frequenzen darstellen
	for (i = 0; i < DibHeight(); i++) {
		t = (BYTE*)GetPixelAddress(0, i);
...
//+ fftbackcopy
// kopieren von rücktransformierten Werten zurück in das dib,
// ggf. schwarzen randauslassen
// übergeben werden real-Werte (imag ist 0), Farbkanal, Breite
// und Höhe des fouriertransformierten Bildes
