...
bool MCIObject::OpenFile(LPCWSTR pszFileName)
{
...
bool MCIObject::OpenAudioCD(LPCWSTR drive, BYTE& tracks) {
	DWORD flag = 0;
	if (m_op.wDeviceID != 0) Close();
...
bool MCIObject::SetAviPosition(HWND hwnd, CRect rect) {
...
bool MCIObject::TMSFSeek(BYTE track, BYTE min, BYTE sek, BYTE frame) {
...
bool MCIObject::GetTrackLength(BYTE track, BYTE& min, BYTE& sek, BYTE& frame) {
...
bool MCIObject::getPlayed() {
	return played;
}

bool MCIObject::GetTMSFPosition(BYTE& track, BYTE& min, BYTE& sek, BYTE& frame) {
